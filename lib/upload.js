...
// Upload a MENU file (image or PDF) to leads/<id>/menu/ via the signed-upload
// path (handles any size + PDFs). Returns the stored menuPhoto record.
export async function uploadLeadMenuFile(leadId, file) {
  const sign = await fetch('/api/uploads/sign', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ scope: 'lead', kind: 'menu', leadId, filename: file.name, contentType: file.type }),
  })
  const sd = await jsonOrThrow(sign, 'Could not start menu upload')
  await putToStorage(sd.uploadUrl, sd.contentType, file)
  const rec = await fetch(`/api/leads/${leadId}/menu-photo-record`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ path: sd.path, name: file.name, contentType: file.type, size: file.size }),
  })
  const rd = await jsonOrThrow(rec, 'Could not record menu upload')
  return rd.menuPhoto
}
...
// Upload an affiliate avatar (image) to affiliates/<id>/ via the signed-upload
// pipeline. Returns a durable public URL (also saved on the affiliate doc).
export async function uploadAffiliateAvatar(affiliateId, file) {
  const sign = await fetch('/api/uploads/sign', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ scope: 'affiliate', affiliateId, filename: file.name, contentType: file.type }),
  })
  const sd = await jsonOrThrow(sign, 'Could not start avatar upload')
  await putToStorage(sd.uploadUrl, sd.contentType, file)
  const fin = await fetch(`/api/affiliates/${affiliateId}/avatar-finalize`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ path: sd.path }),
  })
  const fd = await jsonOrThrow(fin, 'Could not finalize avatar')
  return fd.url
}
