...
// Reply-to routing → monitored inboxes (never noreply). Env-overridable.
// Affiliate-facing mail replies to the partnerships inbox; restaurant/lead
// mail replies to the general inbox. From stays the Resend-verified sender.
const AFFILIATE_REPLY_TO =
  process.env.RESEND_AFFILIATE_REPLY_TO || "affiliates@foodlensgroup.com";
const LEAD_REPLY_TO =
  process.env.RESEND_LEAD_REPLY_TO || "info@foodlensgroup.com";

// Absolute base for email images/links (defaults so the <img> always resolves
// even if PUBLIC_BASE_URL isn't set in Vercel).
const PUBLIC_BASE_URL = (process.env.PUBLIC_BASE_URL || "https://demo.foodlensgroup.com").replace(/\/$/, "");

// The lockup is a transparent PNG — orange "FoodLens" + charcoal "GROUP" — so
// it sits directly on the light email card and both words stay legible. (No
// dark band: charcoal "GROUP" would disappear on black.)
function logoBandHtml() {
  return `<div style="margin-bottom:28px;">
    <img src="${PUBLIC_BASE_URL}/foodlens-logo.png" alt="FoodLens Group" height="30" style="display:block;height:30px;width:auto;border:0;outline:none;text-decoration:none;" />
  </div>`;
}
            ...
            <tr>
              <td>
                ...
                <p style="margin:24px 0 0;color:#4A3F35;font-size:14px;line-height:1.5;">
                  Have a question in the meantime? Just reply to this email — our team reads every reply.
                </p>
...
export async function sendLeadNotification({
  ...
  const replyAddr = email && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) ? email : LEAD_REPLY_TO;
...
export async function sendAffiliateApplicationReceived({ to, name }) {
  ...
  const bodyHtml = `
    ...
    <p style="margin:0 0 0;color:#4A3F35;font-size:14px;line-height:1.5;">Questions? Just reply to this email — it reaches our partnerships team.</p>`;
  const text = [
    ...
    "Questions? Just reply to this email — it reaches our partnerships team.", "", "— The FoodLens team",
  ].join("\n");
  try {
    const res = await client.emails.send({ from: FROM, to, replyTo: AFFILIATE_REPLY_TO, subject, html: affiliateShell({ title: subject, bodyHtml }), text });
...
<html><body style="margin:0;padding:24px;background:#FBF6EE;font-family:-apple-system,Inter,Arial,sans-serif;">
  ...
  const replyAddr = email && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) ? email : AFFILIATE_REPLY_TO;
  try {
  ...
  const bodyHtml = `
    ...
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 8px;">
      <tr><td style="border-radius:9999px;background:#E96A3A;">
        <a href="${kitUrl}" style="display:inline-block;padding:12px 24px;color:#ffffff;font-weight:700;font-size:15px;text-decoration:none;">Open your Affiliate Dashboard →</a>
      </td></tr>
    </table>
    <p style="margin:16px 0 0;color:#4A3F35;font-size:15px;line-height:1.5;">Start with the quick-start in your dashboard to unlock your scripts, assets and tracking link — then <a href="${fieldUrl}" style="color:#E96A3A;font-weight:600;">add your first restaurant</a>. Keep your code handy — it's how every lead you bring in gets credited to you.</p>
    <p style="margin:16px 0 0;color:#4A3F35;font-size:14px;line-height:1.5;">Questions? Just reply to this email — it reaches our partnerships team.</p>`;
  const text = [
    ...
    `Open your Affiliate Dashboard (start here): ${kitUrl}`,
    `Add a restaurant lead: ${fieldUrl}`, "",
    "Keep your code handy — it's how every lead you bring in gets credited to you.", "",
    "Questions? Just reply to this email — it reaches our partnerships team.", "", "— The FoodLens team",
  ].join("\n");
  try {
    const res = await client.emails.send({ from: FROM, to, replyTo: AFFILIATE_REPLY_TO, subject, html: affiliateShell({ title: subject, bodyHtml }), text });
